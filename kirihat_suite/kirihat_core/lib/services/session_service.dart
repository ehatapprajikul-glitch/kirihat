...
class SessionService {
  ...
  // New keys for guest mode and customer mode
  static const String _KEY_IS_GUEST = 'is_guest';
  static const String _KEY_GUEST_PINCODE = 'guest_pincode';
  static const String _KEY_GUEST_AREA = 'guest_area';
  static const String _KEY_GUEST_VENDOR_IDS = 'guest_vendor_ids';
  static const String _KEY_CUSTOMER_MODE = 'force_customer_mode';
  ...
  /// Save guest session (without user ID) - for anonymous browsing
  Future<void> saveGuestSession({
    ...
    // Save location data WITHOUT userId (guest mode)
    await prefs.setString(_KEY_PINCODE, pincode);
    await prefs.setString(_KEY_AREA, area);
    await prefs.setStringList(_KEY_VENDOR_IDS, finalVendorIds);
    
    if (finalVendorIds.isNotEmpty) {
      await prefs.setString(_KEY_VENDOR_ID, finalVendorIds.first);
    }
    
    print('Guest session saved: P:$pincode, A:$area, Vs:$finalVendorIds');
  }

  /// Check if current session is for a guest (no userId saved)
  Future<bool> isGuestSession() async {
    final prefs = await SharedPreferences.getInstance();
    final userId = prefs.getString(_KEY_USER_ID);
    final hasLocation = prefs.getString(_KEY_PINCODE) != null;
    return userId == null && hasLocation;
  }
  ...
  // Customer Mode (Overrides Role)
  Future<void> setCustomerMode(bool isCustomer) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(_KEY_CUSTOMER_MODE, isCustomer);
  }

  Future<bool> isCustomerMode() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getBool(_KEY_CUSTOMER_MODE) ?? false;
  }
}
