...
class _LoginScreenState extends State<LoginScreen> {
  ...
  // Brand Colors
  final Color _brandColor = Colors.blue; // Rider is Blue
  final Color _buttonColor = Colors.blue.shade600;
  ...
  void _navigateBasedOnRole(String roleRaw, BuildContext context) {
    ...
    if (role == 'rider') {
        Navigator.pushAndRemoveUntil(
          context,
          // Go to AuthWrapper which will see the 'rider' role and show dashboard
          MaterialPageRoute(builder: (context) => const AuthWrapper()),
          (route) => false,
        );
    } else {
        ScaffoldMessenger.of(context).showSnackBar(
           SnackBar(content: Text("Access Denied: You are a $role, not a Rider."), backgroundColor: Colors.red),
        );
