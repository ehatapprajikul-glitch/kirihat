...
class _EnhancedSellerInventoryScreenState extends State<EnhancedSellerInventoryScreen> {
          ...
          title: Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.inventory_2, color: Color(0xFF0D9759), size: 24),
              ),
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                // Custom choice chips row
                Row(
                  children: [
                    Expanded(
                      child: _buildUpdateTypeChip(
                        'set',
                        updateType,
                        'Set To',
                        Icons.edit_outlined,
                        Colors.blue,
                        () => setDialogState(() => updateType = 'set'),
                      ),
                    ),
                    ...
                    Expanded(
                      child: _buildUpdateTypeChip(
                        ...
                        Colors.green,
                        () => setDialogState(() => updateType = 'increase'),
                      ),
                    ),
                    const SizedBox(width: 8),
                    Expanded(
                      child: _buildUpdateTypeChip(
                        ...
                        Colors.red,
                        () => setDialogState(() => updateType = 'decrease'),
                      ),
                    ),
                  ],
                ),
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    labelText: updateType == 'set' ? 'New Stock Value' : 'Quantity',
  ...
  // Helper widget for update type selection chips
        ...
        children: [
          ...
          if (isMedium)
             ...
             Wrap(
               ...
               children: [
                 SizedBox(
                   width: (constraints.maxWidth - 32 - 8) / 2, // 2 columns (32 padding + 8 spacing)
                   child: _buildStatCard('In Stock', '${stats['inStock']}', Colors.green, Icons.check_circle),
                 ),
                 SizedBox(
                   width: (constraints.maxWidth - 32 - 8) / 2,
                   child: _buildStatCard('Low Stock', '${stats['lowStock']}', Colors.orange, Icons.warning_amber_rounded),
                 ),
                 SizedBox(
                   width: (constraints.maxWidth - 32 - 8) / 2,
  ...
  // List view (optimized replacement for DataTable)
  Widget _buildTableView(List<Map<String, dynamic>> filteredList, BoxConstraints constraints) {
    // Tighter layout for mobile
    final bool isSmall = constraints.maxWidth < 600;
    final double horizontalPadding = isSmall ? 4.0 : 16.0;
    final double cardMargin = isSmall ? 4.0 : 16.0;
    ...
    return Card(
      ...
      child: filteredList.isEmpty
          ...
          : ListView.builder(
              ...
              itemCount: filteredList.length + 2, // +2 for toolbar and header
              itemBuilder: (context, index) {
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                              flex: 3,
                              child: InkWell(
                                onTap: () => _onSort(0, !_sortAscending),
                                child: Row(
                                  children: [
                                    Flexible(child: Text('Product', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 12), overflow: TextOverflow.ellipsis)),
                            ...
                            if (constraints.maxWidth > 800)
                              const Expanded(
                                flex: 1,
                                child: Text('Status', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12), overflow: TextOverflow.ellipsis),
                              ),
                            // Stock (Sortable)
                            Expanded(
                              flex: 1,
                              child: InkWell(
                                onTap: () => _onSort(2, !_sortAscending),
                                child: Row(
                                  children: [
                                    Flexible(child: Text('Stk', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 12), overflow: TextOverflow.ellipsis)),
                            ...
                            Expanded(
                              flex: 1,
                              child: InkWell(
                                onTap: () => _onSort(3, !_sortAscending),
                                child: Row(
                                  children: [
                                    Flexible(child: Text('Selling', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 12), overflow: TextOverflow.ellipsis)),
                            ...
                            // Actions
                             const SizedBox(width: 84, child: Text('Actions', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12), overflow: TextOverflow.ellipsis)),
                          ],
                        ),
                      ),
                      const Divider(height: 1),
                ...
                return Column(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    _buildInventoryListRow(filteredList[itemIndex], constraints, horizontalPadding),
                    if (itemIndex < filteredList.length - 1)
                      const Divider(height: 1),
                  ],
                );
              },
  ...
  Widget _buildInventoryListRow(Map<String, dynamic> item, BoxConstraints constraints, double padding) {
    final String name = item['name'] ?? 'Unknown';
    final String barcode = item['barcode']?.toString() ?? 'N/A';
    final String image = (item['images'] != null && (item['images'] as List).isNotEmpty) 
        ? item['images'][0] 
        : '';
    final num stockNum = item['stock_quantity'] ?? 0;
    final double mrp = ((item['mrp']) as num? ?? 0).toDouble();
    final double sellingPrice = ((item['selling_price'] ?? item['price']) as num? ?? mrp).toDouble();
    final double discount = mrp > 0 ? ((mrp - sellingPrice) / mrp * 100) : 0;
    final double profit = sellingPrice - ((item['cost_price'] as num?)?.toDouble() ?? 0);
          ...
          child: Container(
            ...
            child: Row(
              children: [
                ...
                // Selling Price with profit/discount
                Expanded(
                  flex: 1,
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
